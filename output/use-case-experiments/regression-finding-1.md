...
**Live verification update (2026-07-20):** VERIFIED end-to-end with a Cohere trial key (the new-user path). Via local stdio server with `COHERE_API_KEY` set: `check_cohere_status` routed to Cohere API `command-a-plus-05-2026`, and `analyze_contract_with_cohere(AB-2026-04073)` returned a complete analysis — `Source: Alberta Purchasing Connection`, `Provider: Cohere API` — with fit (CoR, digital bid bond, heavy-civil scope), opportunity sizing (1.2M m³ excavation, 57,000 t asphalt), risks, and next actions. No key persisted to any file. Reproduction script: `output/use-case-experiments/test_cohere_live.py`.
