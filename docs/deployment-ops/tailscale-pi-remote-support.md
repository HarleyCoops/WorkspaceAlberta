...
2. Revoke the WorkspaceAlberta subscriber key, so the key on the returned disk
   is dead before the box leaves your hands. One lease is one terminal, so
   there is never another device still relying on it.
