...
- `codespace_generator.py` - generates complete GitHub Codespace configurations.
...
- Override the OpenAPI wrapper command/args in code (see `DEFAULT_OPENAPI_WRAPPER_*`) or pass options if you import the helpers.
...
```python
from generator import load_catalog, select_tools, build_mcp_json, build_env_example, build_integrations_md
...
```
Use `write_workspace_files(root, selection)` to emit the three files to disk.

## GitHub automation sketch
1. Collect `github_username`, `repo_name`, and `selected_ids` from the UI.
2. `tools = select_tools(load_catalog(), selected_ids)`
3. Build content: `mcp.json`, `.env.example`, `INTEGRATIONS.md`, `README.md` stub.
4. Create repo from template (PyGithub), add files in one commit, push, invite collaborator.
...
python generator/codespace_generator.py ./output "My Business Workspace" "John Doe" stripe google_calendar github
```

### Programmatic Usage

```python
from codespace_generator import write_codespace_files, CodespaceConfig
...
write_codespace_files(Path("./output"), config, "myorg/my-workspace-repo")
```
